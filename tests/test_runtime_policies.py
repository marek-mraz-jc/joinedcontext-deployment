...
PORTAL_SA = "system:serviceaccount:dev:portal"
BUILD_RULES = {"portal-jobs-are-walled-app-builds", "portal-app-builds-are-bounded"}


def build_job(tmp_path: Path, *, label: str = "app-builder", account: str = "app-builder",
              token: bool = False, deadline: int | None = 900) -> Path:
    job = {
        "apiVersion": "batch/v1",
        "kind": "Job",
        "metadata": {"name": "build-helsinki-bikes", "namespace": "dev",
        ...
        "spec": {
            "template": {
                ...
                "spec": {
                    ...
                    "restartPolicy": "Never",
                    "containers": [{"name": "build", "image": "ghcr.io/x/app-builder@sha256:" + "0" * 64}],
                },
            },
        },
    }
...
@requires_kyverno
@requires_helm
def test_a_build_job_the_portal_starts_inside_its_walls_is_admitted(pod_security, tmp_path):
    """AP-80, AP-81: the Portal's build Job, labelled for its NetworkPolicy, token-less and
    bounded, passes."""
    failed = failed_rules(pod_security, build_job(tmp_path), PORTAL_SA, tmp_path)
    assert not BUILD_RULES & failed, failed
...
@pytest.mark.parametrize(
    ...
    ids=["no NetworkPolicy label", "the Portal's own identity", "a token mounted", "no deadline", "a day long"],
)
def test_a_job_the_portal_starts_outside_the_walls_is_refused(pod_security, tmp_path, wrong):
    """AP-81: a Job the Portal creates without the app-builder label (so no NetworkPolicy
    selects it), under another identity, with a token or unbounded runs an application's code
    with more than a build may have, and is refused."""
    failed = failed_rules(pod_security, build_job(tmp_path, **wrong), PORTAL_SA, tmp_path)
    assert BUILD_RULES & failed, failed


@requires_kyverno
@requires_helm
def test_a_job_another_identity_creates_is_not_judged_by_the_build_rule(pod_security, tmp_path):
    """The rule is about the Portal's Jobs; the bucket bootstrap and the CNPG initdb Job are
    created by others and keep their own labels."""
    failed = failed_rules(pod_security, build_job(tmp_path, label="bucket", deadline=None),
                          JOB_CONTROLLER_SA, tmp_path)
    assert not BUILD_RULES & failed, failed
